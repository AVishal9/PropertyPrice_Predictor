...
outlier_subset <- subset(stratified_sample, select = c(price, house_size, land_size))
...
#Now removing extreme values from the stratified_sample 
...
# Add `status` column
stratified_sample <- stratified_sample %>%
  mutate(status = sample(c("sold", "for_sale"), nrow(stratified_sample), replace = TRUE))

# Create dummy variables
status_dummies <- dummy_cols(stratified_sample, select_columns = "status") %>%
  select(-status, -status_for_sale)  # Drop `status` and one dummy column to avoid multicollinearity
...
stratified_sample$zip_code <- ifelse(stratified_sample$zip_code %in% zip_stats_less_than_19 , 'other', stratified_sample$zip_code)
...
stratified_sample$city <- ifelse(stratified_sample$city %in% city_stats_less_than_100, 'other', stratified_sample$city)
...
city <- unique(stratified_sample$city) #6012 unique Variables before grouping.After grouping 13
...
street <- unique(stratified_sample$street) #28412 unique values. 8 after grouping 
...
zip_code <- unique(stratified_sample$zip_code) #10602 unique values. 9 after grouping 
...
data2 <- subset(stratified_sample, select = -c(status, state, city, street, zip_code))
data2 <- cbind(data2, state_dummies, status_dummies,city_dummies,street_dummies,zipcode_dummies)
...
>>>>>>> b7b5fd91fd9f64c81a4a92747da7e0e7758c6b9e
